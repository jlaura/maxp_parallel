...
def initialize(job,z,w,floor,maxattempts=250): #if I can figureout how to represent w as an array it will need to be initailzed as a global and removed as an arg.
    ...
    while solving and attempts <= maxattempts:
        ...
        if not seeds:
            ...
            nonseeds = [i for i in w.id_order if i not in seeds] #So a seed is a potential soln.  Here are we checking to see what indices are not in the seed?  If we can store these in a numpy array a function might exist that is faster than a for loop.
            candidates = seeds
            candidates.extend(nonseeds) #Here we get the missing indices concatenated with the existing list.
        
        while candidates:
            seed = candidates.pop(0) #What are we doing here?  I have read that ifilter from itertools can do what .pop does with no overhead when the lists are large.
            ...
            while building_region:
                ...
                if check_floor(region): #Here we are seeing if we surpass the min number of elements - 3 in tests.
...
#Allocate space TODO: Write into a function
...
#Setup a shared mem array for p with dim = n
sharedP = mp.RawArray(ctypes.c_double, numP*cores)
numpyP = np.frombuffer(sharedP)
numpyP.shape = (numP,cores)
numpyP[:] = 1
initP(numpyP) #initialize the solution space as a shared memory array
...
for job in range(cores): #Prep to assign each core a job
    pool.apply_async(initialize, args=(job,z,w,floor,)) #Async apply each job
pool.close()
pool.join()
